...
todos:
  ...
  - id: community-api-client
    content: 创建 frontend-v2/lib/community-api.ts，完整封装所有第一阶段和第二阶段的社区 API 调用
    status: completed
...
**文件**: `backend-v2/alembic/versions/20260226_1200_add_community_groups.py`
...
**文件**: `backend-v2/app/models/community_group.py`
...
在 `backend-v2/app/models/__init__.py` 中导出。

### 3. 更新 CommunityService（`[backend-v2/app/services/community_service.py](backend-v2/app/services/community_service.py)`）
...
### 4. Pydantic Schemas（`[backend-v2/app/schemas/community.py](backend-v2/app/schemas/community.py)`）
...
### 5. 更新 API 路由（`[backend-v2/app/api/community.py](backend-v2/app/api/community.py)`）
...
### 1. 匿名发布开关（`[frontend-v2/app/(app)/dreams/new/page.tsx](frontend-v2/app/(app)`/dreams/new/page.tsx)）
...
**文件**: `frontend-v2/app/(app)/community/explore/page.tsx`
...
- `frontend-v2/app/(app)/community/greenhouse/page.tsx` — 社群列表页，展示所有官方社群
- `frontend-v2/app/(app)/community/greenhouse/[slug]/page.tsx` — 单个社群详情页，含该社群的梦境流

社群卡片包含：社群图标、名称、描述、成员数、帖子数、"加入"按钮。

### 4. 更新社区主页（`[frontend-v2/app/(app)/community/page.tsx](frontend-v2/app/(app)`/community/page.tsx)）
...
**文件**: `frontend-v2/lib/community-api.ts`

当前此文件不存在，但已被引用。创建完整的 API 客户端，包含所有第一阶段 + 第二阶段端点的封装。

### 6. 情绪标签完善（`[frontend-v2/components/community/dream-card-social.tsx](frontend-v2/components/community/dream-card-social.tsx)`）
...
### 7. i18n 翻译更新（`[frontend-v2/i18n/index.ts](frontend-v2/i18n/index.ts)`）
