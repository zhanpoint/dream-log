...
## 第一阶段:基础 UI 组件 (frontend-v2)

### 1. 复制模板组件到项目

从 `magic-ui-template/dillionverma-startup-template` 复制以下组件到 [`frontend-v2/components/ui/`](frontend-v2/components/ui/):
...
在 [`frontend-v2/components/auth/`](frontend-v2/components/auth/) 创建:
...
在 [`frontend-v2/i18n/index.ts`](frontend-v2/i18n/index.ts) 添加认证相关的翻译:
...
在 [`frontend-v2/lib/auth-api.ts`](frontend-v2/lib/auth-api.ts) 创建:
...
更新 [`frontend-v2/lib/api.ts`](frontend-v2/lib/api.ts):
...
在 [`frontend-v2/app/(auth)/auth/page.tsx`](frontend-v2/app/\\\\\\\\\\(auth)/auth/page.tsx):
...
在 [`frontend-v2/app/(auth)/auth/callback/google/page.tsx`](frontend-v2/app/\\\\\\\\\\(auth)/auth/callback/google/page.tsx):
...
在 [`frontend-v2/app/(auth)/layout.tsx`](frontend-v2/app/\\\\\\\\\\(auth)/layout.tsx):
...
在 [`frontend-v2/lib/auth-schemas.ts`](frontend-v2/lib/auth-schemas.ts) 使用 Zod 定义:
...
在 [`frontend-v2/lib/auth-error-handler.ts`](frontend-v2/lib/auth-error-handler.ts):
...
已在 `frontend-v2/package.json` 中包含:
