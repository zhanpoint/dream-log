...
- **隐私等级**：`Dream` 模型已存在 `PRIVATE / FRIENDS / PUBLIC` 枚举字段（[backend/app/models/enums.py](backend/app/models/enums.py)）
- **用户主页**：`User` 模型已有 `username`、`avatar`、`bio` 字段（[backend/app/models/user.py](backend/app/models/user.py)）
- **通知系统**：基于 SSE + Redis Pub/Sub 的实时通知已完整实现（[backend/app/models/notification.py](backend/app/models/notification.py)）
...
**梦境创建**（[backend/app/api/dreams.py](backend/app/api/dreams.py)）：
...
在 [frontend/components/site-header.tsx](frontend/components/site-header.tsx) 主导航栏中新增"梦境广场"入口（作为社区总入口）：
...
在 [frontend/i18n/index.ts](frontend/i18n/index.ts) 中新增 `community` 命名空间，包含：
