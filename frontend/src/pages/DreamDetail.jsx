...
import { ArrowLeft, Calendar, Edit, Trash2, Sparkles, Moon, Hash, Bed, Sun, Star, Brain, Heart, Repeat, BookOpen, Shield, Users, Globe, Save, X, PenLine } from 'lucide-react';
...
const DreamDetail = () => {
    ...
    const { user } = useAuth();
    const { t, formatDate } = useI18nContext();

    // 初始化翻译配置
    const categoryConfig = getCategoryConfig(t);
    const moodConfig = getMoodConfig(t);
    const sleepQualityConfig = getSleepQualityConfig(t);
    const privacyConfig = getPrivacyConfig(t);
    ...
    const handleSaveInterpretation = async () => {
        ...
        try {
            ...
            notification.success(t('common.success.saved', '保存成功'));
        } catch (error) {
            notification.error(t('common.error.saveFailed', '保存失败') + ': ' + (error.response?.data?.message || error.message));
        } finally {
    ...
    return (
        <>
            <MultilingualSeo
                title={dream?.title ? `${dream.title} | ${t('dreams:detail.title', '梦境详情')}` : t('dreams:detail.title', '梦境详情')}
                description={dream?.content ? dream.content.substring(0, 150) : t('dreams:detail.subtitle', '查看梦境详细内容和分析')}
                keywords={t('dreams:detail.keywords', '梦境,详情,分析')}
            />
            <div className="dream-detail-container">
                {/* 页面头部 */}
                <div className="dream-detail-header">
                    ...
                    >
                        ...
                        <div className="overview-actions">
                            ...
                            >
                                ...
                                {t('dreams:detail.aiAnalysis', 'AI分析')}
                            </Button>
                                        ...
                                        <AlertDialogContent>
                                            <AlertDialogHeader>
                                                ...
                                                <AlertDialogDescription>
                                                    {t('dreams:detail.deleteConfirmation', '确定要删除这个梦境记录吗？此操作无法撤销')}
                                                </AlertDialogDescription>
                                            </AlertDialogHeader>
                                            <AlertDialogFooter>
                        ...
                        <div className="overview-content">
                            ...
                            <div className="dream-meta-row">
                                ...
                                <Badge variant="outline" className="privacy-badge">
                                    {React.createElement(privacyConfig[dream.privacy]?.icon, { className: 'h-3 w-3' })}
                                    {privacyConfig[dream.privacy]?.label}
                                </Badge>
                            </div>
                            ...
                            <div className="dream-tags-row">
                                ...
                                {dream.categories?.map(category => {
                                    ...
                                    const config = categoryConfig[categoryName] || { label: categoryName, color: '#6b7280' };
                                    return (
                                        <Badge
                                ...
                                {dream.is_recurring && (
                                    <Badge
                                        ...
                                        variant="secondary"
                                        title={dream.recurring_elements ? `${t('dreams:detail.recurringElements', '重复元素')}: ${dream.recurring_elements}` : t('dreams:detail.recurringDescription', '这是一个重复出现的梦境')}
                                    >
                                        <Repeat className="h-3 w-3" />
                                        {t('dreams:detail.recurringDream', '重复梦境')}
                                        {dream.recurring_elements && (
                                            <span className="recurring-hint">({t('dreams:detail.hoverDetails', '悬停查看详情')})</span>
                                        )}
                                    </Badge>
                                )}
                    ...
                    >
                        <div className="section-header">
                            <h3 className="section-title">
                                ...
                                {t('dreams:detail.sleepEmotion', '睡眠与情绪')}
                            </h3>
                        </div>
                            ...
                            <CardContent className="emotion-compact-content">
                                ...
                                {(dream.mood_before_sleep || dream.mood_in_dream || dream.mood_after_waking) && (
                                    <div className="emotion-track">
                                        <h4 className="track-title">
                                            ...
                                            {t('dreams:detail.emotionTrack', '情绪轨迹')}
                                        </h4>
                                        <div className="emotion-stages">
                                            {dream.mood_before_sleep && (
                                                <div className="emotion-stage">
                                                    <Moon className="h-5 w-5" />
                                                    <span className="stage-label">{t('dreams:detail.emotion.beforeSleep', '睡前')}</span>
                                                    <span className="mood-text">{moodConfig[dream.mood_before_sleep]?.label}</span>
                                                </div>
                                            )}
                                            {dream.mood_in_dream && (
                                                <div className="emotion-stage">
                                                    <Sparkles className="h-5 w-5" />
                                                    <span className="stage-label">{t('dreams:detail.emotion.inDream', '梦中')}</span>
                                                    <span className="mood-text">{moodConfig[dream.mood_in_dream]?.label}</span>
                                                </div>
                                            )}
                                            {dream.mood_after_waking && (
                                                <div className="emotion-stage">
                                                    <Sun className="h-5 w-5" />
                                                    <span className="stage-label">{t('dreams:detail.emotion.afterWaking', '醒后')}</span>
                                                    <span className="mood-text">{moodConfig[dream.mood_after_waking]?.label}</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="sleep-metrics-row">
                                    <h4 className="metrics-category-title">
                                        ...
                                        {t('dreams:detail.sleepStatus', '睡眠状态')}
                                    </h4>
                                    <div className="metrics-row-content">
                                        {dream.sleep_quality && (
                                            <div className="metric-item compact">
                                                <div className="metric-header">
                                                    <span>{t('dreams:detail.sleepQuality', '睡眠质量')}</span>
                                                </div>
                                                <div className="metric-value">
                                                    <div className="stars-compact">
                                                        {renderSleepQualityStars(dream.sleep_quality)}
                                                    </div>
                                                    <span className="quality-text">{sleepQualityConfig[dream.sleep_quality]?.label}</span>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {(dream.bedtime || dream.wake_time) && (
                                            <div className="metric-item compact sleep-times-item">
                                                <div className="metric-header">
                                                    <span>{t('dreams:detail.sleepTime', '睡眠时间')}</span>
                                                </div>
                                                <div className="metric-value">
                                        ...
                                        {dream.sleep_duration && (
                                            <div className="metric-item compact sleep-duration-item">
                                                <div className="metric-header">
                                                    <span>{t('dreams:detail.sleepDuration', '睡眠时长')}</span>
                                                </div>
                                                <div className="metric-value">
                                ...
                                <div className="emotion-metrics-row">
                                    <h4 className="metrics-category-title">
                                        ...
                                        {t('dreams:detail.perception', '感知体验')}
                                    </h4>
                                    <div className="metrics-row-content">
                                        <div className="metric-item compact">
                                            <div className="metric-header">
                                                <span>{t('dreams:detail.lucidity', '清醒度')}</span>
                                            </div>
                                            <div className="metric-value">
                                        ...
                                        {dream.vividness && (
                                            <div className="metric-item compact">
                                                <div className="metric-header">
                                                    <span>{t('dreams:detail.clarity', '清晰度')}</span>
                                                </div>
                                                <div className="metric-value">
                    ...
                    {dream.personal_notes && (
                        ...
                        >
                            <div className="section-header">
                                <h3 className="section-title">
                                    <BookOpen className="h-5 w-5" />
                                    {t('dreams:detail.personalNotes', '个人笔记')}
                                </h3>
                            </div>
                    ...
                    >
                        <div className="section-header">
                            <h3 className="section-title">
                                ...
                                {t('dreams:detail.aiAnalysis', 'AI智能分析')}
                            </h3>
                        </div>
                            ...
                            <CardHeader>
                                <div className="card-header-content">
                                    <h3 className="card-title">
                                        ...
                                        {t('dreams:detail.myAnalysis', '我的解析')}
                                    </h3>
                                    {isAuthor && !isEditingInterpretation && (
                                        ...
                                        >
                                            ...
                                            {userInterpretation ? t('common.edit', '编辑') : t('dreams:detail.addAnalysis', '添加解析')}
                                        </Button>
                                    )}
                                </div>
                            </CardHeader>
                                ...
                                {isEditingInterpretation ? (
                                    <div className="interpretation-editor">
                                        <Textarea
                                            ...
                                            placeholder={t('dreams:detail.analysisPlaceholder', '参考AI分析结果，结合你的生活经历，写下你对这个梦境的理解和感悟...')}
                                        ...
                                        <div className="editor-actions">
                                            ...
                                            >
                                                ...
                                                {isSavingInterpretation ? t('common.saving', '保存中...') : t('common.save', '保存')}
                                            </Button>
                                        </div>
                                    </div>
                                ) : (
                                                ...
                                                <div className="empty-state">
                                                    <div className="empty-state-content">
                                                        ...
                                                        <div className="empty-state-text">
                                                            <h4>{t('dreams:detail.analysisPrompt', '参考AI分析结果，结合你的生活经历，记录你对这个梦境的独特见解')}</h4>
                                                        </div>
                                                    </div>
                                                </div>
                                            )
                    ...
                    {dream.updated_at !== dream.created_at && (
                        <div className="content-footer">
                            <span className="update-time">
                                {t('common.lastUpdated', '更新于')} {new Date(dream.updated_at).toLocaleString()}
                            </span>
                        </div>
                    )}
                </div>
